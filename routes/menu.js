...
menuRoute.post("/", authenticate, async (request, response) => {
  ...
  if (
    ...
    if (result.length == 0) {
      ...
      if (resultAdd) {
        resObj.message = "added!";
        resObj.object = newProduct;
      }
    } else resObj.message = 'item "id" or "title" already occupied';
  } else
    resObj.message =
      "invalid product, product needs 'id', 'title', 'desc' and 'price'";
  response.json(resObj);
});
...
menuRoute.delete("/", authenticate, async (request, response) => {
  ...
  if (
    ...
    if (result >= 1) {
      ...
      resObj.message = "item title not found";
    }
  } else
    resObj.message =
      "invalid product, product needs 'id', 'title', 'desc' and 'price'";
